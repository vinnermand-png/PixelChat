...
type Tool = "paint" | "erase" | "erasePlatform" | "place" | "eraseObject";
...
export default function GameMakerV2() {
  ...
  useEffect(() => { const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext("2d"); if (!ctx) return; ctx.imageSmoothingEnabled = false; ctx.clearRect(0, 0, VIEW_W, VIEW_H); ctx.fillStyle = "#15202b"; ctx.fillRect(0, 0, VIEW_W, VIEW_H); drawFoundation(ctx, world.terrain, world.gridSize, edgeMaterial, edgeDepth); drawTerrainSurface(ctx, world.terrain, world.gridSize); drawObjects(ctx, objects); if (showGrid) drawGridOverlay(ctx, world.gridSize); drawHover(ctx, hover); }, [world, objects, hover, showGrid, edgeMaterial, edgeDepth]);
  ...
  function handleCell(cell: Cell) {
    const key = cellKey(cell.gx, cell.gy);
    if (tool === "paint" || tool === "erase") { setWorld((current) => { const terrain = { ...current.terrain }; if (tool === "paint") terrain[key] = "grass"; else delete terrain[key]; return { ...current, terrain }; }); return; }
  ...
  return <main style={{ maxWidth: 1120, margin: "0 auto", padding: 24, color: "#e8eef7" }}><header style={{ marginBottom: 18 }}><div style={{ color: "#7f95aa", fontSize: 12, letterSpacing: 2 }}>PIXELCHAT · GAME MAKER V2</div><h1 style={{ margin: "6px 0 8px" }}>STEP 6 — OBJECT SYSTEM + TERRAIN ERASE</h1><p style={{ margin: 0, color: "#9fb0c2" }}>Objects remain separate from terrain and foundation. ERASE PLATFORM removes one connected terrain area.</p></header><section style={{ display: "grid", gridTemplateColumns: "220px minmax(0, 1fr)", gap: 18 }}><aside style={{ border: "1px solid #33475c", background: "#101820", padding: 14 }}><h2 style={{ fontSize: 14, marginTop: 0 }}>TERRAIN TOOLS</h2><button onClick={() => setTool("paint")} style={{ width: "100%", marginBottom: 8, padding: 10, fontWeight: tool === "paint" ? 800 : 400 }}>PAINT GRASS</button><button onClick={() => setTool("erase")} style={{ width: "100%", marginBottom: 8, padding: 10, fontWeight: tool === "erase" ? 800 : 400 }}>ERASE CELL</button><button onClick={() => setTool("erasePlatform")} style={{ width: "100%", marginBottom: 16, padding: 10, fontWeight: tool === "erasePlatform" ? 800 : 400 }}>ERASE PLATFORM</button><h2 style={{ fontSize: 14 }}>OBJECT TOOLS</h2><button onClick={() => setTool("place")} style={{ width: "100%", marginBottom: 8, padding: 10, fontWeight: tool === "place" ? 800 : 400 }}>PLACE TEST TREE</button><button onClick={() => setTool("eraseObject")} style={{ width: "100%", marginBottom: 16, padding: 10, fontWeight: tool === "eraseObject" ? 800 : 400 }}>ERASE OBJECT</button><button onClick={() => setShowGrid((current) => !current)} style={{ width: "100%", marginBottom: 16, padding: 10, fontWeight: 800 }}>GRID · {showGrid ? "ON" : "OFF"}</button><button onClick={clearWorld} style={{ width: "100%", padding: 10 }}>CLEAR WORLD</button><h2 style={{ fontSize: 14, marginTop: 24 }}>FOUNDATION</h2>{(["soil", "rock", "cliff"] as EdgeMaterial[]).map((material) => <button key={material} onClick={() => setEdgeMaterial(material)} style={{ width: "100%", marginBottom: 8, padding: 10, fontWeight: edgeMaterial === material ? 800 : 400 }}>{material.toUpperCase()}</button>)}<label style={{ display: "block", marginTop: 8, color: "#9fb0c2", fontSize: 13 }}>FOUNDATION DEPTH · {edgeDepth}px<input type="range" min={MIN_EDGE_DEPTH} max={MAX_EDGE_DEPTH} value={edgeDepth} onChange={(event) => setEdgeDepth(Number(event.target.value))} style={{ width: "100%", marginTop: 8 }} /></label><div style={{ marginTop: 22, color: "#9fb0c2", fontSize: 13, lineHeight: 1.6 }}><div><b>TERRAIN CELLS:</b> {Object.keys(world.terrain).length}</div><div><b>OBJECTS:</b> {objects.length}</div><div><b>TOOL:</b> {tool.toUpperCase()}</div><div><b>HOVER:</b> {hover ? `${hover.gx}, ${hover.gy}` : "OUTSIDE WORLD"}</div></div></aside><section style={{ border: "1px solid #33475c", background: "#0b1118", padding: 12 }}><canvas ref={canvasRef} width={VIEW_W} height={VIEW_H} onPointerMove={(event) => setHover(getCell(event))} onPointerLeave={() => setHover(null)} onPointerDown={(event) => { const cell = getCell(event); if (cell) handleCell(cell); }} style={{ width: "100%", height: "auto", display: "block", cursor: "crosshair", imageRendering: "pixelated", touchAction: "none" }} /></section></section></main>;
}
