...
export default function GameMakerV2() {
  ...
  const enterPlayMode = () => { const first = Object.keys(world.terrain).map((key) => key.split(",").map(Number)).find(([gx, gy]) => inBounds(gx, gy, world.gridSize)); if (!first) { setMapStatus("Paint terrain before starting Play Test"); return; } setSelectedObjectId(null); setHover(null); setPlayer((current) => current && world.terrain[cellKey(current.gx, current.gy)] ? current : { gx: first[0], gy: first[1] }); setMode("play"); };
  const movePlayer = (dx: number, dy: number) => setPlayer((current) => { if (!current || mode !== "play") return current; const next = { gx: current.gx + dx, gy: current.gy + dy }; return world.terrain[cellKey(next.gx, next.gy)] ? next : current; });
  ...
  useEffect(() => { const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext("2d"); if (!ctx) return; ctx.imageSmoothingEnabled = false; ctx.fillStyle = "#15202b"; ctx.fillRect(0, 0, VIEW_W, VIEW_H); renderWorld(ctx); }, [world, objects, player, hover, showGrid, edgeMaterial, edgeDepth, selectedObject, mode]);

  const getCell = (event: React.PointerEvent<HTMLCanvasElement>) => { const rect = event.currentTarget.getBoundingClientRect(); return screenToCell(((event.clientX - rect.left) / rect.width) * VIEW_W, ((event.clientY - rect.top) / rect.height) * VIEW_H, world.gridSize); };
  const erasePlatform = (start: Cell) => { const startKey = cellKey(start.gx, start.gy); if (!world.terrain[startKey]) return; const terrain = { ...world.terrain }, visited = new Set<string>(), queue: Cell[] = [start]; while (queue.length) { const cell = queue.pop()!, key = cellKey(cell.gx, cell.gy); if (visited.has(key) || !terrain[key]) continue; visited.add(key); delete terrain[key]; [[1,0],[-1,0],[0,1],[0,-1]].forEach(([dx, dy]) => queue.push({ gx: cell.gx + dx, gy: cell.gy + dy })); } commitChange({ ...world, terrain }, objects.filter((object) => !visited.has(cellKey(object.gx, object.gy)))); };
  const deleteSelectedObject = () => { if (!selectedObjectId) return; commitChange(world, objects.filter((object) => object.id !== selectedObjectId)); setTool("select"); };
  const handleCell = (cell: Cell) => { if (mode !== "edit") return; const key = cellKey(cell.gx, cell.gy); if (tool === "paint") { if (!world.terrain[key]) commitChange({ ...world, terrain: { ...world.terrain, [key]: "grass" } }, objects); return; } if (tool === "erase") { if (world.terrain[key]) { const terrain = { ...world.terrain }; delete terrain[key]; commitChange({ ...world, terrain }, objects.filter((object) => object.gx !== cell.gx || object.gy !== cell.gy)); } return; } if (tool === "erasePlatform") { erasePlatform(cell); return; } if (tool === "select") { setSelectedObjectId(objects.find((object) => object.gx === cell.gx && object.gy === cell.gy)?.id || null); return; } if (tool === "move") { if (!selectedObjectId || !world.terrain[key] || objects.some((object) => object.id !== selectedObjectId && object.gx === cell.gx && object.gy === cell.gy)) return; commitChange(world, objects.map((object) => object.id === selectedObjectId ? { ...object, gx: cell.gx, gy: cell.gy } : object)); setTool("select"); return; } if (tool === "place" && world.terrain[key] && !objects.some((object) => object.gx === cell.gx && object.gy === cell.gy)) { commitChange(world, [...objects, { id: `${Date.now()}-${cell.gx}-${cell.gy}`, assetId: selectedAssetId, gx: cell.gx, gy: cell.gy }]); return; } if (tool === "eraseObject") { const next = objects.filter((object) => object.gx !== cell.gx || object.gy !== cell.gy); if (next.length !== objects.length) commitChange(world, next); } };
  const clearWorld = () => { if (mode !== "edit" || (!Object.keys(world.terrain).length && !objects.length)) return; if (window.confirm("Clear the entire world? This can be undone.")) { commitChange({ ...world, terrain: {} }, []); setPlayer(null); setTool("paint"); } };
  const buttonStyle = (active = false): React.CSSProperties => ({ width: "100%", marginBottom: 8, padding: "9px 10px", border: active ? "1px solid #ffd84a" : "1px solid #33475c", background: active ? "#25354a" : "#15202b", color: "#e8eef7", borderRadius: 8, textAlign: "left", cursor: "pointer" });
  const panel: React.CSSProperties = { border: "1px solid #33475c", background: "#101820", borderRadius: 12, padding: 14 };
  const playButton: React.CSSProperties = { padding: "10px 14px", borderRadius: 8, border: "1px solid #54c978", background: mode === "play" ? "#2f7d32" : "#17261d", color: "#e8eef7", fontWeight: 800, cursor: "pointer" };

  const categoryButtons: Array<[BuildCategory, string]> = [["terrain", "🌱 Terrain"], ["objects", "🌳 Objects"], ["decorations", "✨ Decorations"], ["foundation", "🪨 Foundation"]];
  return <main style={{ maxWidth: 1400, margin: "0 auto", padding: 20, color: "#e8eef7" }}>
    <header style={{ ...panel, marginBottom: 14, padding: 12, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
      <div style={{ fontWeight: 900, letterSpacing: 1 }}>PIXELCHAT GAME MAKER V2</div>
      <input value={mapName} disabled={mode === "play"} onChange={(event) => { setMapName(event.target.value); setMapStatus("Not saved"); }} style={{ minWidth: 180, padding: 9, borderRadius: 7 }} />
      <span style={{ color: "#9fb0c2", fontSize: 13 }}>{mode === "play" ? "PLAY TEST" : mapStatus}</span>
      <div style={{ marginLeft: "auto", display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button disabled={mode === "play" || !history.past.length} onClick={undo}>↶ Undo</button><button disabled={mode === "play" || !history.future.length} onClick={redo}>↷ Redo</button><button disabled={mode === "play"} onClick={saveMap}>Save</button><button disabled={mode === "play"} onClick={loadSavedMap}>Load</button>
        {mode === "edit" ? <button onClick={enterPlayMode} style={playButton}>▶ PLAY TEST</button> : <button onClick={() => setMode("edit")} style={playButton}>✎ EDIT MODE</button>}
      </div>
    </header>
    <section style={{ display: "grid", gridTemplateColumns: "230px minmax(0, 1fr) 250px", gap: 14, alignItems: "start" }}>
      <aside style={panel}>
        <div style={{ fontSize: 12, letterSpacing: 1.5, color: "#7f95aa", marginBottom: 10 }}>BUILD TOOLS</div>
        {mode === "play" ? <div style={{ color: "#9fb0c2", lineHeight: 1.5 }}>Play Test is active.<br />Use <b>WASD</b> or <b>Arrow Keys</b> to move.</div> : <>
          {categoryButtons.map(([category, label]) => <button key={category} onClick={() => setActiveCategory(category)} style={buttonStyle(activeCategory === category)}>{label}</button>)}
          <hr style={{ borderColor: "#33475c", margin: "12px 0" }} />
          {activeCategory === "terrain" && <><button onClick={() => setTool("paint")} style={buttonStyle(tool === "paint")}>Paint Grass</button><button onClick={() => setTool("erase")} style={buttonStyle(tool === "erase")}>Erase Cell</button><button onClick={() => setTool("erasePlatform")} style={buttonStyle(tool === "erasePlatform")}>Erase Platform</button></>}
          {activeCategory === "objects" && <><div style={{ color: "#9fb0c2", fontSize: 12, marginBottom: 8 }}>ASSET LIBRARY</div>{ASSET_LIBRARY.filter((asset) => asset.category === "nature").map((asset) => <button key={asset.id} onClick={() => { setSelectedAssetId(asset.id); setTool("place"); }} style={buttonStyle(tool === "place" && selectedAssetId === asset.id)}>🌳 {asset.name}</button>)}<button onClick={() => setTool("select")} style={buttonStyle(tool === "select")}>Select</button><button onClick={() => setTool("eraseObject")} style={buttonStyle(tool === "eraseObject")}>Delete Object</button><button disabled={!selectedObject} onClick={() => setTool("move")} style={buttonStyle(tool === "move")}>Move Selected</button></>}
          {activeCategory === "decorations" && <div style={{ color: "#9fb0c2", lineHeight: 1.5 }}>Decoration assets will appear here as the library grows.</div>}
          {activeCategory === "foundation" && <><div style={{ marginBottom: 8 }}>Material</div>{(["soil", "rock", "cliff"] as EdgeMaterial[]).map((material) => <button key={material} onClick={() => { setEdgeMaterial(material); setMapStatus("Not saved"); }} style={buttonStyle(edgeMaterial === material)}>{material.toUpperCase()}</button>)}<label style={{ display: "block", color: "#9fb0c2", marginTop: 10 }}>Depth · {edgeDepth}px<input type="range" min={MIN_EDGE_DEPTH} max={MAX_EDGE_DEPTH} value={edgeDepth} onChange={(event) => { setEdgeDepth(Number(event.target.value)); setMapStatus("Not saved"); }} style={{ width: "100%" }} /></label></>}
        </>}
      </aside>
      <section style={{ ...panel, padding: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, color: "#9fb0c2", fontSize: 13 }}><span>{mode === "play" ? "PLAY TEST · Player stays on terrain" : "EDIT MODE"}</span>{mode === "edit" && <button onClick={() => setShowGrid((current) => !current)}>GRID {showGrid ? "ON" : "OFF"}</button>}</div>
        <canvas ref={canvasRef} width={VIEW_W} height={VIEW_H} onPointerMove={(event) => mode === "edit" && setHover(getCell(event))} onPointerLeave={() => setHover(null)} onPointerDown={(event) => { const cell = getCell(event); if (cell) handleCell(cell); }} style={{ width: "100%", height: "auto", display: "block", imageRendering: "pixelated", cursor: mode === "edit" ? "crosshair" : "default" }} />
      </section>
      <aside style={panel}>
        <div style={{ fontSize: 12, letterSpacing: 1.5, color: "#7f95aa", marginBottom: 10 }}>INSPECTOR</div>
        {mode === "play" && <><b>TEST PLAYER</b><div style={{ color: "#9fb0c2", marginTop: 8 }}>Cell: {player ? `${player.gx}, ${player.gy}` : "Not placed"}</div><div style={{ color: "#9fb0c2", marginTop: 8 }}>Movement: WASD / Arrow Keys</div><div style={{ color: "#9fb0c2", marginTop: 8 }}>Depth: {player ? getPlayerDepth(player) : "—"}</div></>}
        {mode === "edit" && selectedObject && <><b>{getAsset(selectedObject.assetId)?.name}</b><div style={{ color: "#9fb0c2", marginTop: 8 }}>Cell: {selectedObject.gx}, {selectedObject.gy}</div><div style={{ color: "#9fb0c2", marginTop: 8 }}>Depth: {getObjectDepth(selectedObject)}</div><button onClick={deleteSelectedObject} style={{ marginTop: 12 }}>Delete</button></>}
        {mode === "edit" && !selectedObject && activeCategory === "foundation" && <><b>FOUNDATION</b><div style={{ color: "#9fb0c2", marginTop: 8 }}>Material: {edgeMaterial}</div><div style={{ color: "#9fb0c2", marginTop: 8 }}>Depth: {edgeDepth}px</div></>}
        {mode === "edit" && !selectedObject && activeCategory !== "foundation" && <><b>MAP</b><div style={{ color: "#9fb0c2", marginTop: 8 }}>Terrain cells: {Object.keys(world.terrain).length}</div><div style={{ color: "#9fb0c2", marginTop: 8 }}>Objects: {objects.length}</div><div style={{ color: "#9fb0c2", marginTop: 8 }}>ID: {mapId}</div><button onClick={clearWorld} style={{ marginTop: 18, color: "#ffb0b0" }}>Clear World…</button></>}
      </aside>
    </section>
  </main>;
}
